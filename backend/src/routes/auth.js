...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (!['creator', 'brand', 'admin'].includes(role)) {
      ...
      return res.status(400).json({ error: "Invalid role specified. Must be 'creator', 'brand', or 'admin'" });
    }
...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    console.log(`Reset link: ${process.env.FRONTEND_URL || 'http://localhost:3000'}/reset-password/${token}`);
